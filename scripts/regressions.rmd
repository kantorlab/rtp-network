*Note*: 
- Best to run this file in the r console in RStudio using the run button above. 
- Running it in vscode will need the pandoc module to be loaded, creating an html. But the html may not be rendered directly.
...
partner_db <- readRDS(here("derived_data", "partner_db.rds"))

partner_db_non_missing_studyidto <- readRDS(here("derived_data", "partner_db_non_missing_studyidto.rds"))


genomic_db_allcols_sequenced <- readRDS(here("derived_data", "genomic_db_sequenced_dt.rds"))
...
```{r}
genomic_db_allcols_sequenced$StudyID <- factor(genomic_db_allcols_sequenced$StudyID)
genomic_db_allcols_sequenced$DemoGender <- factor(genomic_db_allcols_sequenced$DemoGender)
genomic_db_allcols_sequenced$DemoRace <- factor(genomic_db_allcols_sequenced$DemoRace)
genomic_db_allcols_sequenced$DemoHispanic <- factor(genomic_db_allcols_sequenced$DemoHispanic)
genomic_db_allcols_sequenced$DemoBirthCountry <- factor(genomic_db_allcols_sequenced$DemoBirthCountry)
genomic_db_allcols_sequenced$RiskPrimary <- factor(genomic_db_allcols_sequenced$RiskPrimary)
genomic_db_allcols_sequenced$RiskMSM <- factor(genomic_db_allcols_sequenced$RiskMSM)
genomic_db_allcols_sequenced$RiskIDU <- factor(genomic_db_allcols_sequenced$RiskIDU)
genomic_db_allcols_sequenced$RiskHeterosexual <- factor(genomic_db_allcols_sequenced$RiskHeterosexual)
genomic_db_allcols_sequenced$Sequence <- factor(genomic_db_allcols_sequenced$Sequence)
genomic_db_allcols_sequenced$SequenceSubtype <- factor(genomic_db_allcols_sequenced$SequenceSubtype)
genomic_db_allcols_sequenced$ClusteredHIVTrace005 <- factor(genomic_db_allcols_sequenced$ClusteredHIVTrace005)
genomic_db_allcols_sequenced$ClusteredHIVTrace015 <- factor(genomic_db_allcols_sequenced$ClusteredHIVTrace015)
genomic_db_allcols_sequenced$ClusteredPhyloAny <- factor(genomic_db_allcols_sequenced$ClusteredPhyloAny)
```
...
genomic_db <- 
  ...
  select(StudyID, DemoGender, DemoRace, DemoHispanic, 
         ...
         mutate(Sequence = droplevels(Sequence))
table(genomic_db$Sequence)
```
...
```{r}
source(here("utils", "create-edgelists-from-mol-clusters.R"))
source(here("utils", "regressions-functions.R"))
source(here("utils", "split-counts-by-attributes.R"))
```

## Recode Sequence Subtype as B/non-B (needed for regressions):

```{r}
genomic_db$SequenceSubtype <- as.character(genomic_db$SequenceSubtype)
...
genomic_db_w_index_persons_in_clusteredphyloany <- 
  genomic_db[index_person_idx_in_clusteredphyloany,]
...
phyloany_named_pt_res #distribution of naming and not naming molecular cluster members 
...
m1_clean_dt <- 
  genomic_db_w_index_persons_in_clusteredphyloany %>%
  select(StudyID, named_partner_indicator, ClusteredPhyloAny, DemoGender) %>%
  filter(DemoGender != "") %>%
...
m1 <- geem(as.numeric(as.character(named_partner_indicator)) ~ 
               DemoGender, 
           data = m1_clean_dt, 
           #data = genomic_db_w_index_persons_in_clusteredphyloany, #DOES NOT CONVERGE WITH THIS
            family = "binomial", 
           id=ClusteredPhyloAny, 
           corstr="exchangeable")
summary(m1)

genomic_db_w_index_persons_in_clusteredphyloany$DemoRace <- 
  relevel(genomic_db_w_index_persons_in_clusteredphyloany$DemoRace, ref = "Black")
...
## Predict Molecular Clustering in Named Partnerships
...
named_pts_not_in_clusteredphyloany <- which(!(named_pt_net %v% "vertex.names") %in% index_persons_in_clusteredphyloany)
length(named_pts_not_in_clusteredphyloany)
```
...
who are naming or being named as partners in partner DB. OF these,
...
table(individuals_in_named_pt_net_dt$DemoGender, 
      exclude = NULL)
...
m1_clean_dt_converse <- 
   individuals_in_named_pt_net_dt  %>%
  select(StudyID, indicator_phyloany_cluster, ClusteredPhyloAny, DemoGender) %>%
  filter(DemoGender != "") %>%
   mutate(DemoGender = droplevels(DemoGender))

table(m1_clean_dt_converse$DemoGender, exclude = NULL)

m1_converse <- geem(as.numeric(as.character(indicator_phyloany_cluster)) ~ 
               DemoGender, 
           data = m1_clean_dt_converse, 
            family = "binomial", 
           id=ClusteredPhyloAny, 
           corstr="exchangeable")
summary(m1_converse)
...
m2_clean_dt_converse <- 
   individuals_in_named_pt_net_dt  %>%
  select(StudyID, indicator_phyloany_cluster, ClusteredPhyloAny, DemoRace) %>%
  #filter(DemoRace != "") %>%
   mutate(DemoRace = droplevels(DemoRace)) %>%
   mutate(DemoRace = relevel(DemoRace, ref = "Black"))

table(m2_clean_dt_converse$DemoRace, exclude = NULL)

m2_converse <- geem(as.numeric(as.character(indicator_phyloany_cluster)) ~ 
               DemoRace, 
           data = m2_clean_dt_converse, 
            family = "binomial", 
           id=ClusteredPhyloAny, 
           corstr="exchangeable")
summary(m2_converse)
...
m3_clean_dt_converse <- individuals_in_named_pt_net_dt %>%
  select(StudyID, indicator_phyloany_cluster, ClusteredPhyloAny, RecodedSequenceSubtype) %>%
  #filter(RecodedSequenceSubtype != "") %>%
   mutate(RecodedSequenceSubtype = factor(RecodedSequenceSubtype)) %>% # Convert to factor first
   mutate(RecodedSequenceSubtype = relevel(RecodedSequenceSubtype, ref = "B"))

table(m3_clean_dt_converse$RecodedSequenceSubtype, exclude = NULL)

m3_converse <- geem(as.numeric(as.character(indicator_phyloany_cluster)) ~ 
               RecodedSequenceSubtype, 
           data = m3_clean_dt_converse, 
            family = "binomial", 
           id=ClusteredPhyloAny, 
           corstr="exchangeable")
summary(m3_converse)
...
m4_clean_dt_converse <- 
   individuals_in_named_pt_net_dt  %>%
  select(StudyID, indicator_phyloany_cluster, ClusteredPhyloAny, RiskMSM) %>%
  #filter(RiskMSM != "") %>%
  mutate(RiskMSM = droplevels(RiskMSM))

table(m4_clean_dt_converse$RiskMSM, exclude = NULL)

m4_converse <- geem(as.numeric(as.character(indicator_phyloany_cluster)) ~ 
               RiskMSM, 
           data = m4_clean_dt_converse, 
            family = "binomial", 
           id=ClusteredPhyloAny, 
           corstr="exchangeable")
summary(m4_converse)
...
m5_clean_dt_converse <- 
   individuals_in_named_pt_net_dt  %>%
  select(StudyID, indicator_phyloany_cluster, ClusteredPhyloAny, RiskHeterosexual) %>%
  #filter(RiskHeterosexual != "") %>%
   mutate(RiskHeterosexual = droplevels(RiskHeterosexual))

table(m5_clean_dt_converse$RiskHeterosexual, exclude = NULL)

m5_converse <- geem(as.numeric(as.character(indicator_phyloany_cluster)) ~ 
               RiskHeterosexual, 
           data = m5_clean_dt_converse, 
            family = "binomial", 
           id=ClusteredPhyloAny, 
           corstr="exchangeable")
summary(m5_converse)
...
m6_clean_dt_converse <- 
   individuals_in_named_pt_net_dt  %>%
  select(StudyID, indicator_phyloany_cluster, ClusteredPhyloAny, RiskIDU) %>%
 # filter( RiskIDU!= "") %>%
   mutate( RiskIDU= droplevels(RiskIDU))

table(m6_clean_dt_converse$RiskIDU, exclude = NULL)

m6_converse <- geem(as.numeric(as.character(indicator_phyloany_cluster)) ~ 
               RiskIDU, 
           data = m6_clean_dt_converse, 
            family = "binomial", 
           id=ClusteredPhyloAny, 
           corstr="exchangeable")
summary(m6_converse)
...
m7_clean_dt_converse <- individuals_in_named_pt_net_dt %>%
  select(StudyID, indicator_phyloany_cluster, ClusteredPhyloAny, HIVDxAge) %>%
  mutate(HIVDxAge = as.numeric(HIVDxAge))

m7_converse <- geem(as.numeric(as.character(indicator_phyloany_cluster)) ~ 
               HIVDxAge, 
           data = m7_clean_dt_converse, 
            family = "binomial", 
           id=ClusteredPhyloAny, 
           corstr="exchangeable")
summary(m7_converse)
