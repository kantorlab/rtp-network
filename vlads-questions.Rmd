...
### StudyIDFrom 
Table produced from the perspective of the StudyIDFrom (i.e., index persons)

How many contacts are reported by each interviewed participant 
(i.e., "StudyIDFrom") at each interview?

Mutuality?? Cross-Naming - i.e., person who is nominated by someone also
nominates them?
...
net_dt[,.N, by=c("StudyIDFrom", "ReferralDate", "StudyIDTo")][order(ReferralDate)]

########
# Organize this by year of referral as opposed to the full date
########
```
```{r}
xtabs(data = net_dt, 
...
### StudyIDTo 
Table produced from the perspective of the StudyIDTo (i.e., contacts nominated by index persons)
...
# contacts_reported_peryear_by_indices_wfill <- 
#   dcast(net_dt[substr(StudyIDFrom, 1, 1) != "C"],
#       StudyIDTo ~ substr(ReferralDate, 1, 4),
#       value.var = "StudyIDFrom",
#       fun.aggregate = length,
...
# In this case 0 indicates either missing information 
# (i.e., peson was not in the dataset or may have not reported anybody.)
```

The matrix `contacts_reported_peryear_by_indices` creates the number of index persons that nominate a given
contact in a year. (The zeros do niot differentiate between "no one reported" and 
"contact not in the database".)

Study IDs reported by more than 1 partner (not necessarily in different years):
...
Study IDs reported by at least one index person in multiple years: 
```{r}
...
are not the same. The number of persons reporting these Study IDs 
as partners in a given year are not the same as the number of years in which
these Study IDs are reported as a partners, i.e., multiple persons report these 
...
**Next Steps**:    
  - Identify the same for Study Id From.   
  - Before recruitment date of Study ID To, Study ID From, code as NAs.  

**Send the data to Vlad at the link he shares later. I will need to create a data dictionary of the datasets that I create for him.**
